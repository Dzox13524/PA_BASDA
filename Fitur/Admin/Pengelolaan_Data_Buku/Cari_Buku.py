    ...
    while loop:
        ...
        if len(result["cocok"]) > 0:
            ...
            for idx, res1 in enumerate(result["cocok"]):
                ...
                print(f"    Ketersediaan   : {data1['Ketersediaan']}")
                buttons_parameter.append({"Nama": "Hapus Buku " + data1["JudulBuku"], "command":"d" + str(idx +1), "function":lambda i=data1["ISBN"] : Fitur_Hapus_Buku(i)})
                buttons_parameter.append({"Nama": "Tambah Stok Buku " + data1["JudulBuku"], "command":"t" + str(idx +1), "function":lambda i=data1["ISBN"] : Fitur_Tambah_Stok_Buku(i)})
        else:
